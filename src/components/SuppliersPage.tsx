...
export function SuppliersPage({ mode }: { mode: "admin" | "branch" }) {
  ...
  const scoped = useMemo(
    () =>
      suppliers.filter((supplier) => {
        ...
        return `${supplier.companyName} ${supplier.email ?? ""} ${supplier.phone ?? ""}`
          .toLowerCase()
  ...
  const stats = {
    ...
    outstanding: scoped.reduce(
      (sum, s) =>
        sum + (s.balanceType === "Payable" ? s.openingBalanceAmount : -s.openingBalanceAmount),
      0,
    ),
  };
              ...
              return (
                ...
                >
                  ...
                  <td className="px-5 py-3 text-muted-foreground">
                    {supplier.email || "-"}
                    <div className="num text-xs">{supplier.phone}</div>
                  </td>
                  {isAdmin && (
                  ...
                  <td className="px-5 py-3 text-right num">
                    {fmtMoney(supplier.openingBalanceAmount)}
                  </td>
                      ...
                      onClick={async () => {
                        if (
                          !(await confirm({
                            ...
                            description: `Are you sure you want to delete supplier ${supplier.companyName}?`,
                          }))
                        )
                          return;
                        try {
...
function SupplierDialog({
  ...
  const [openingBalanceAmount, setOpeningBalanceAmount] = useState(
    String(initial?.openingBalanceAmount ?? 0),
  );
  const [balanceType, setBalanceType] = useState<"Payable" | "Receivable">(
    initial?.balanceType ?? "Payable",
  );
      ...
      >
        <div className="mb-5 flex items-start justify-between">
          <div>
            ...
            <h2 className="font-display text-2xl">{initial ? "Edit supplier" : "Add supplier"}</h2>
          </div>
        ...
        <form className="grid gap-5" onSubmit={submit}>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <label className="grid gap-1.5">
              ...
              <div className="flex gap-3 rounded-md border border-border bg-background px-3 py-2 text-sm">
                <label>
                  <input
                    type="radio"
          ...
          <div className="grid gap-4 sm:grid-cols-4">
            ...
            <SelectField
              ...
              value={country}
              onChange={setCountry}
              options={["India", "United States", "United Kingdom", "UAE", "Australia"]}
            />
          </div>
          <div className="grid gap-4 sm:grid-cols-3">
            <Field
              ...
              helper="Any existing balance with this supplier"
            />
            <SelectField
              label="Balance type"
              value={balanceType}
              onChange={(v) => setBalanceType(v as "Payable" | "Receivable")}
              options={["Payable", "Receivable"]}
              helper={balanceType === "Payable" ? "Payable (We Owe)" : "Receivable (They Owe Us)"}
            />
            <Field
              label="Balance as of date"
              type="date"
              value={balanceAsOfDate}
              onChange={setBalanceAsOfDate}
            />
            <SelectField
              label="Payment terms"
              value={paymentTerm}
            ...
            <Field
              ...
              helper="Maximum credit allowed for this supplier"
            />
          </div>
