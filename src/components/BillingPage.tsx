...
export function BillingPage({ mode }: { mode: "admin" | "branch" }) {
  ...
  const issue = async () => {
    ...
    if (!isAdmin && branchId !== session?.branchId) {
      toast.error("You can only bill from your assigned branch");
      return;
    }
    if (items.length === 0) {
  ...
  return (
    <>
      ...
      <PageHeader
        ...
        title="New bill"
        description={isAdmin ? "Create sales for any branch with required branch assignment." : "Fast, branch-scoped billing."}
      />
          ...
          <div className="border-b border-border p-4">
            ...
            <div className="relative">
              ...
              <input
                ...
                disabled={!branchId}
                className="w-full rounded-md border border-border bg-background py-2.5 pl-9 pr-3 text-sm outline-none focus:border-ink disabled:opacity-50"
              />
            </div>
          </div>
          <ul className="divide-y divide-border/60">
            ...
            {filtered.length === 0 && (
              <li className="px-5 py-10 text-center text-sm text-muted-foreground">
                {branchId ? "No products found." : "Select a branch to see products."}
              </li>
            )}
          </ul>
        </div>
        ...
        <aside className="rounded-xl border border-border bg-card">
          <div className="border-b border-border p-4">
            <div className="text-[10px] uppercase tracking-widest text-muted-foreground">
              Current bill {branch ? `· ${branch.name}` : ""}
            </div>
            <input
            ...
            <select
              ...
              className="mt-3 w-full rounded-md border border-border bg-transparent px-2 py-2 text-sm outline-none focus:border-ink"
            >
              {String(import.meta.env.VITE_PAYMENT_METHODS || "Cash,Card,UPI").split(",").map((pm: string) => (
                <option key={pm} value={pm.trim()}>
                  {pm.trim()}
                </option>
              ))}
            </select>
            ...
            {items.map((it) => (
              <div key={it.productId} className="flex flex-wrap items-center gap-3 border-b border-border/60 px-4 py-3 sm:flex-nowrap sm:px-5">
                ...
                <div className="flex items-center gap-1.5">
                  <button type="button" onClick={() => setQty(it.productId, it.qty - 1)} className="rounded-md border border-border p-1 hover:bg-accent">
                ...
                <button type="button" onClick={() => setQty(it.productId, 0)} className="rounded-md p-1 text-muted-foreground hover:text-destructive">
          ...
          <div className="space-y-2 border-t border-border p-5 text-sm">
            ...
            <button
              type="button"
              onClick={issue}
              disabled={items.length === 0 || !branchId}
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-ink/40 p-4 backdrop-blur-sm no-print" onClick={() => setIssued(null)}>
          <div className="mx-auto my-8 max-w-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="mb-3 flex justify-between">
              <button type="button" onClick={() => window.print()} className="inline-flex items-center gap-2 rounded-md bg-paper px-3 py-1.5 text-sm text-ink hover:opacity-90">
                <Printer className="size-4" /> Print
              </button>
              <button type="button" onClick={() => setIssued(null)} className="rounded-md bg-paper p-1.5 text-ink">
                <X className="size-4" />
              </button>
            </div>
            <InvoiceDocument bill={issued} branch={branches.find((b) => b.id === issued.branchId)} />
