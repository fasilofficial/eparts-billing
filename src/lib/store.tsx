...
const seed = (): StoreState => {
  ...
  const b3 = { id: "br_3", name: "Harbor Outlet", email: "harbor@billing.app", password: "branch123", address: "4 Pier Road", createdAt: new Date().toISOString() };
  const products: Product[] = [];
  const names = ["Espresso Blend 250g", "Ceramic Mug", "Cold Brew Bottle", "Artisan Croissant", "Matcha Tin", "Reusable Tote", "Drip Filter Pack", "Single Origin 1kg"];
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const value: StoreCtx = {
    ...
    addBranch: (b) =>
      setState((s) => ({
        ...s,
        branches: [...s.branches, { ...b, id: `br_${Date.now()}`, createdAt: new Date().toISOString() }],
      })),
    ...
    addProduct: (p) => setState((s) => ({ ...s, products: [...s.products, { ...p, id: `p_${Date.now()}` }] })),
    ...
    addBill: (b) => {
      ...
      const bill: Bill = { ...b, id: `bl_${Date.now()}`, number, createdAt: new Date().toISOString() };
      setState((s) => {
        // decrement stock
