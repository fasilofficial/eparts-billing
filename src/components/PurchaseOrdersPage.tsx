          ...
          onSave={async (data) => {
            try {
              ...
              if (editing) {
                ...
                toast.success(data.status === "Draft" ? "Draft saved" : "Purchase order created");
              }
              closeDialog();
            } catch (e: any) {
...
function PurchaseDialog({
  ...
  onClose: () => void;
  onSave: (data: Omit<PurchaseOrder, "id" | "number" | "createdAt">) => void;
}) {
  ...
  const submit = async (status: "Draft" | "Created") => {
    ...
    onSave({
      ...
      additionalCharges: charges.filter((c) => c.label || c.amount),
  ...
  return (
    ...
    >
      <div
        className="mx-auto my-6 w-full max-w-6xl rounded-xl border border-border bg-card p-6 shadow-paper"
        ...
        <form className="grid gap-5" onSubmit={(e: FormEvent) => e.preventDefault()}>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <ComboSelect
              label="Supplier *"
              value={supplierInput}
              onText={setSupplierInput}
              selected={supplierId}
              onSelect={setSupplierId}
              options={branchSuppliers.map((s) => ({ id: s.id, label: s.companyName }))}
            />
            <Field
              label="Purchase date *"
              type="date"
                  ...
                  <tr key={index} className="border-b border-border/60">
                    ...
                    <td className="px-3 py-2">
                      <MiniInput
                        ...
                        onChange={(value) => updateItem(index, { tax: Number(value) || 0 })}
                      />
                    </td>
                    <td className="px-3 py-2">
                      <MiniInput
                        ...
                        onChange={(value) =>
                          updateItem(index, { discountPercent: Number(value) || 0 })
                        }
                      />
                    </td>
          ...
          <button
            ...
            className="inline-flex w-fit items-center gap-2 rounded-md border border-border px-3 py-2 text-sm hover:bg-accent"
          >
                          ...
                          {isUrl ? (
                            isPdf ? (
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="size-full flex flex-col items-center justify-center p-1 text-[10px] text-center text-muted-foreground hover:bg-accent transition"
                                title="View PDF"
                              >
                            ...
                            <div
                              className="size-full flex flex-col items-center justify-center p-1 text-[8px] text-center text-muted-foreground break-all"
                              title={att}
                            >
                              <span>📄</span>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            />
            <Field label="Shipping details" value={shippingDetails} onChange={setShippingDetails} />
          </div>
          <div className="grid gap-2">
            {charges.map((charge, index) => (
              <div key={index} className="grid gap-2 sm:grid-cols-[1fr_10rem_auto]">
                ...
                <Field
                  ...
                  onChange={(value) =>
                    setCharges((prev) =>
                      prev.map((c, i) => (i === index ? { ...c, amount: Number(value) || 0 } : c)),
                    )
                  }
                />
                <button
            ...
            <button
              type="button"
              onClick={() => setCharges((prev) => [...prev, { label: "", amount: 0 }])}
          ...
          <div className="ml-auto w-full max-w-sm rounded-xl border border-border bg-background p-4 text-sm">
            <Row label="Subtotal" value={fmtMoney(subtotal)} />
            <Row label="Shipping" value={fmtMoney(Number(shippingCharge) || 0)} />
            <Row label="Additional charges" value={fmtMoney(chargesTotal)} />
...
function ProductPicker({
  ...
  const selectProduct = (value: string) => {
    const product = products.find(
      (p) => p.name === value || p.sku === value || p.barcode === value,
    );
    if (product)
      onChange({
  ...
  return (
    <div className="relative">
      <input
        ...
        className="w-full rounded-md border border-border bg-card px-2 py-1.5 pr-8 text-sm outline-none focus:border-ink"
      />
      <ScanLine className="absolute right-2 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
      <datalist id={id}>
...
function MiniInput({
  ...
  return (
    <input
      ...
      value={value}
      onChange={(e) => onChange(e.target.value)}
      className="w-20 rounded-md border border-border bg-card px-2 py-1.5 text-sm outline-none focus:border-ink"
    />
  );
}
function TextArea({
